import asyncio
import json

from pipeline.style import style_captions
from schemas.vlm import GlobalFactualSummary


class FakeCaptionClient:
    async def generate_json(self, *, model: str, prompt: str):
        return {
            "captions": {
                "formal": {
                    "style_name": "formal",
                    "caption": "formal caption",
                    "grounded_facts_used": ["fact 1"],
                    "safety_notes": [],
                },
                "sarcastic": {
                    "style_name": "sarcastic",
                    "caption": "sarcastic caption",
                    "grounded_facts_used": ["fact 1"],
                    "safety_notes": [],
                },
                "humorous_tech": {
                    "style_name": "humorous_tech",
                    "caption": "humorous_tech caption",
                    "grounded_facts_used": ["fact 1"],
                    "safety_notes": [],
                },
                "humorous_non_tech": {
                    "style_name": "humorous_non_tech",
                    "caption": "humorous_non_tech caption",
                    "grounded_facts_used": ["fact 1"],
                    "safety_notes": [],
                },
            }
        }


async def _fake_upload_file_async(*, bucket: str, object_path: str, source, content_type: str):
    return object_path


def test_style_captions_generates_all_variants_and_uploads_json(tmp_path, monkeypatch):
    summary_path = tmp_path / "global_factual_summary.json"
    summary_path.write_text(
        GlobalFactualSummary(
            factual_summary="A person opens a laptop and speaks to camera.",
            uncertainties=["Exact brand is unclear."],
        ).model_dump_json(indent=2),
        encoding="utf-8",
    )
    monkeypatch.setattr("pipeline.style.upload_file_async", _fake_upload_file_async)

    final_result, local_path, uploaded_path = asyncio.run(
        style_captions(
            client=FakeCaptionClient(),
            model="test-model",
            job_id="job-1",
            bucket="videos",
            storage_prefix="processed/job-1",
            global_summary_path=summary_path,
            global_summary_storage_path="processed/job-1/global_factual_summary.json",
        )
    )

    assert final_result.formal_caption == "formal caption"
    assert final_result.sarcastic_caption == "sarcastic caption"
    assert final_result.humorous_tech_caption == "humorous_tech caption"
    assert final_result.humorous_non_tech_caption == "humorous_non_tech caption"
    assert uploaded_path == "processed/job-1/final_result.json"
    assert json.loads(summary_path.parent.joinpath("final_result.json").read_text(encoding="utf-8"))["job_id"] == "job-1"
    assert local_path.endswith("final_result.json")
