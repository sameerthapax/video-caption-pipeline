...
from services.supabase_storage import upload_file_async
...
async def style_captions(
    *,
...
) -> tuple[FinalCaptionResult, str, str]:
    ...
    final_result = FinalCaptionResult(
        ...
        source_global_factual_summary_path=global_summary_storage_path,
    ...
    uploaded_path = await upload_file_async(
        bucket=bucket,
        object_path=f"{storage_prefix}/final_result.json",
        source=final_result_path,
        content_type="application/json",
    )
    return final_result, str(final_result_path), uploaded_path
