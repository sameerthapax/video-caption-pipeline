...
- `apps/api`: Django + Django REST Framework backend with placeholder processing logic in `pipeline/`.
...
3. Django stores the file under `apps/api/media/videos/`, creates a `VideoJob`, and starts a background thread.
4. Placeholder pipeline steps update job status and persist a `VideoCaptionResult`.
...
- Background processing uses a thread placeholder instead of Celery, Redis, or a queue.
